export function PeakCluster({ className = "" }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 340 210"
      fill="none"
      aria-hidden
    >
      <path fill="#a8c9b8" d="M168 210 248 28l92 182z" />
      <path fill="#fcfffb" d="M248 28 218 86l30-16 32 16z" />
      <path fill="#d4e4d8" d="M248 28 228 72l20-10 22 10z" />
      <path fill="#1a2a22" d="M12 210 148 42l148 168z" />
      <path fill="#e4ede5" d="M148 42 122 86l26-14 28 14z" />
      <path fill="#6eab86" d="M92 210 168 92l92 118z" opacity=".35" />
      <g transform="translate(140 28)">
        <circle cx="4" cy="3.2" r="2.35" fill="#b06a4a" />
        <path fill="#b06a4a" d="M1.6 6.2h4.8v6.2H1.6z" />
        <path stroke="#b06a4a" strokeWidth="1.35" strokeLinecap="round" d="M2.2 12.6 0.4 18.4M6.2 12.6 8.2 18.4" />
        <path stroke="#1a2a22" strokeWidth="1.2" strokeLinecap="round" d="M8.4 4.2 12.8 19" />
        <circle cx="16.4" cy="4.4" r="2.35" fill="#6eab86" />
        <path fill="#6eab86" d="M14.1 7.4h4.6v6H14.1z" />
        <path stroke="#6eab86" strokeWidth="1.35" strokeLinecap="round" d="M14.6 13.6 13 19.2M18.4 13.6 20.6 19.2" />
      </g>
    </svg>
  );
}

export function WaveCluster({ className = "" }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 360 168"
      fill="none"
      aria-hidden
    >
      <path
        fill="#7bb5a8"
        d="M0 168c28-28 52-62 92-62 32 0 44 24 76 22 34-2 42-38 86-36 36 2 48 30 84 26 14-2 18-12 22-22v134H0z"
        opacity=".9"
      />
      <path
        fill="#5a9488"
        d="M0 168c36-18 64-48 108-44 38 4 46 28 84 24 40-4 52-34 96-30 28 2 42 22 72 18v100H0z"
      />
      <path
        fill="#f3f7f0"
        d="M18 78c18-2 22 16 40 14 16-2 20-18 38-16 14 2 16 14 32 12 12-2 16-16 30-14"
        stroke="#f3f7f0"
        strokeWidth="5"
        strokeLinecap="round"
        opacity=".85"
      />
      <path
        fill="#ffffff"
        d="M96 54c14-1 18 12 32 10 12-2 14-14 28-12"
        stroke="#fff"
        strokeWidth="4"
        strokeLinecap="round"
      />
      <circle cx="54" cy="48" r="3.2" fill="#fff" opacity=".9" />
      <circle cx="168" cy="36" r="2.4" fill="#fff" opacity=".8" />
      <circle cx="214" cy="58" r="2" fill="#fff" opacity=".7" />
      <path
        stroke="#fff"
        strokeWidth="2.4"
        strokeLinecap="round"
        d="M142 28c8 6 6 16-2 20M232 42c10 4 8 14 0 18M76 32c6 8 2 16-6 18"
        opacity=".75"
      />
    </svg>
  );
}

/** Symmetric cosine — start and end share Y, mirrored amplitudes. */
export const WAVES =
  "M0 70 C 240 110, 480 110, 720 70 C 960 30, 1200 30, 1440 70 L1440 90 L0 90 Z";

export function BandEdge({
  kind,
  flip = false,
}: {
  kind?: "mountains" | "waves" | "torn";
  flip?: boolean;
}) {
  void kind;
  return (
    <svg
      className={`ridge-edge ${flip ? "ridge-edge--bottom" : "ridge-edge--top"}`}
      viewBox="0 0 1440 90"
      preserveAspectRatio="none"
      aria-hidden
    >
      <path fill="var(--ridge-fill)" d={WAVES} />
    </svg>
  );
}

export function FooterRidge() {
  return (
    <svg className="ridge-edge--footer" viewBox="0 0 1440 90" preserveAspectRatio="none" aria-hidden>
      <path fill="var(--ivory)" d={WAVES} />
    </svg>
  );
}
