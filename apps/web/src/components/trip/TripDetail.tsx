    ...
    <>
      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-5 py-14 lg:grid-cols-[1fr_320px] lg:px-8">
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-[1.5rem] bg-snow p-6 shadow-[var(--shadow)] ring-1 ring-ink/8">
            ...
            <Link href={bookHref} className="mt-6 block rounded-full bg-copper py-3 text-center text-sm font-medium text-snow hover:bg-copper-deep">
              {raft ? t("bookRaft") : t("bookThis")}
            </Link>
            <a href={wa} className="mt-2 block py-2 text-center text-sm text-moss hover:underline">
              {t("enquire")}
            </a>
          </div>
          {cross && (
            <Link href={tripHref(cross)} className="mt-4 block rounded-[1.4rem] bg-moss-deep p-5 text-snow">
              <p className="text-xs uppercase tracking-[0.16em] text-copper">{raft ? t("pairTrek") : t("finishPokhara")}</p>
      ...
      <div className="sticky bottom-0 z-30 border-t border-ink/10 bg-ivory/95 p-3 backdrop-blur md:hidden">
        <Link href={bookHref} className="block rounded-full bg-copper py-3 text-center text-sm font-medium text-snow">
          {raft ? t("bookRaft") : t("bookThis")}
        </Link>
      </div>
    </>
