  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-2.5 md:grid-cols-[auto_1.4fr_1fr_0.55fr_auto] md:items-end md:gap-3">
        <div className="col-span-2 flex gap-1.5 md:col-span-1 md:pb-0.5">
          {(["trek", "rafting"] as const).map((k) => (
            <button
              key={k}
              type="button"
              onClick={() => chooseKind(k)}
              className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-medium sm:text-sm ${
                kind === k ? "bg-moss text-snow" : "bg-ivory text-ink-soft"
              }`}
            >
              {k === "trek" ? <Mountain className="h-3.5 w-3.5" /> : <Waves className="h-3.5 w-3.5" />}
